...
struct dwmac_priv {
	...
	struct dwmac_dma_ch rx_ch[8];
	struct dwmac_dma_ch tx_ch[8];
...
#define REG_READ(r)     sys_read32(cfg->base_addr + (r))
#define REG_WRITE(r, v) sys_write32((v), cfg->base_addr + (r))
...
struct dwmac_rx_queue {
	uint16_t size;
	uint8_t priority;
	uint8_t dma_channel;
	uint8_t threshold;
	bool sf;
	bool dma_channel_select;
	bool av_queue;
};

struct dwmac_tx_queue {
	uint16_t size;
	uint8_t priority;
	uint8_t threshold;
	bool sf;
...
struct dwmac_config {
	...
	uint8_t rx_queues_to_use;
	uint8_t tx_queues_to_use;
	struct dwmac_rx_queue rx_queues_cfg[8];
	struct dwmac_tx_queue tx_queues_cfg[8];
};
...
#define DWMAC_TX_QUEUE_CONFIG(n)                                                                   \
	{                                                                                          \
		.size = DT_PROP_OR(n, snps_size, 0), .priority = DT_PROP_OR(n, snps_priority, 0),  \
...
#define DWMAC_TX_QUEUES(n)                                                                         \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_DRV_INST(n), txq_config),                                  \
		    (DT_INST_PROP_BY_PHANDLE(n, txq_config, snps_tx_queues_to_use)), (1))

#define DWMAC_TX_DESCRIPTORS(n)                                                                    \
	static struct dwmac_dma_desc dwmac##n##_tx_descs[DWMAC_TX_QUEUES(n)][NB_TX_DESCS] __desc_mem
#define DWMAC_RX_DESCRIPTORS(n)                                                                    \
	static struct dwmac_dma_desc dwmac##n##_rx_descs[DT_INST_PROP_OR(                          \
		n, snps_rx_channel_to_use, 1)][NB_RX_DESCS] __desc_mem

#define DWMAC_IRQ_INIT(n, name)                                                                    \
	extern void dwmac_##name##_isr(const struct device *dev);                                  \
	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(n, name, irq), DT_INST_IRQ_BY_NAME(n, name, priority),     \
		    dwmac_##name##_isr, DEVICE_DT_INST_GET(n),                                     \
...
#define DWMAC_INIT_CONFIG(n)                                                                       \
	...
	{                                                                                          \
		...
		const struct dwmac_config *cfg = dev->config;                                      \
                                                                                                   \
		DWMAC_IRQ_INIT(n, common);                                                         \
		irq_enable(DT_INST_IRQ_BY_NAME(n, common, irq));                                   \
                                                                                                   \
		COND_CODE_1(                                                                       \
			DT_NODE_HAS_PROP(DT_DRV_INST(inst), local_mac_address),                    \
...
#define DWMAC_DEVICE(n)                                                                            \
	DWMAC_INIT_CONFIG(n)                                                                       \
	DWMAC_RX_DESCRIPTORS(n);                                                                   \
	DWMAC_TX_DESCRIPTORS(n);

#define DWMAC_DT_INST_CONFIG(n)                                                                    \
	{                                                                                          \
		...
		.burst_length = DT_INST_PROP(n, snps_burst_length),                                \
		.rx_descs = &dwmac##n##_rx_descs[0][0], .tx_descs = &dwmac##n##_tx_descs[0][0],    \
		.rx_queues_to_use = COND_CODE_1(                                                   \
			DT_NODE_HAS_PROP(DT_DRV_INST(n), rxq_config),                              \
			(DT_INST_PROP_BY_PHANDLE(n, rxq_config, snps_rx_queues_to_use)), (1)),     \
		.tx_queues_to_use = COND_CODE_1(                                                   \
			DT_NODE_HAS_PROP(DT_DRV_INST(n), txq_config),                              \
			(DT_INST_PROP_BY_PHANDLE(n, txq_config, snps_tx_queues_to_use)), (1)),     \
		.rx_queues_cfg = {COND_CODE_1(DT_NODE_EXISTS(DT_INST_CHILD(n, rxq_config)),        \
					      (DT_FOREACH_CHILD_SEP(DT_INST_CHILD(n, rxq_config),  \
								    DWMAC_RX_QUEUE_CONFIG, (, ))), \
					      ())},                                                \
		.tx_queues_cfg = {                                                                 \
			COND_CODE_1(DT_NODE_EXISTS(DT_INST_CHILD(n, txq_config)),                  \
				    (DT_FOREACH_CHILD_SEP(DT_INST_CHILD(n, txq_config),            \
							  DWMAC_TX_QUEUE_CONFIG, (, ))),           \
				    ())                                                            \
		}                                                                                  \
	}
...
#define MAC_RXQ_CTRL0_RXQnEN_POS(n) (2 * n)
enum {
	MAC_RXQ_CTRL0_RXQnEN_DISABLED,
	MAC_RXQ_CTRL0_RXQnEN_AV,
	MAC_RXQ_CTRL0_RXQnEN_GENERIC
};
...
#define MTL_TXQn_OPERATION_MODE_TQS_POS 16
#define MTL_TXQn_OPERATION_MODE_TQS     GENMASK(31, MTL_TXQn_OPERATION_MODE_TQS_POS)
#define MTL_TXQn_OPERATION_MODE_TTC_POS 4
#define MTL_TXQn_OPERATION_MODE_TXQEN   BIT(3)
#define MTL_TXQn_OPERATION_MODE_TSF     BIT(1)
#define MTL_TXQn_OPERATION_MODE_FTQ     BIT(0)

enum {
	MTL_TXQn_OPERATION_MODE_TTC_32,
	MTL_TXQn_OPERATION_MODE_TTC_64,
	MTL_TXQn_OPERATION_MODE_TTC_96,
	MTL_TXQn_OPERATION_MODE_TTC_128,
	MTL_TXQn_OPERATION_MODE_TTC_192,
	MTL_TXQn_OPERATION_MODE_TTC_256,
	MTL_TXQn_OPERATION_MODE_TTC_384,
	MTL_TXQn_OPERATION_MODE_TTC_512
};
...
#define MTL_RXQn_OPERATION_MODE_RQS_POS 20
#define MTL_RXQn_OPERATION_MODE_RQS     GENMASK(31, MTL_RXQn_OPERATION_MODE_RQS_POS)
#define MTL_RXQn_OPERATION_MODE_RFD_POS 14
#define MTL_RXQn_OPERATION_MODE_RFD     GENMASK(19, 14)
#define MTL_RXQn_OPERATION_MODE_RFA_POS 8
#define MTL_RXQn_OPERATION_MODE_RFA     GENMASK(13, MTL_RXQn_OPERATION_MODE_RQS_POS)
#define MTL_RXQn_OPERATION_MODE_RSF     BIT(5)
#define MTL_RXQn_OPERATION_MODE_FEP     BIT(4)
#define MTL_RXQn_OPERATION_MODE_FUP     BIT(3)
#define MTL_RXQn_OPERATION_MODE_RTC_POS 0
#define MTL_RXQn_OPERATION_MODE_RTC     GENMASK(1, MTL_RXQn_OPERATION_MODE_RTC_POS)
enum {
	MTL_RXQn_OPERATION_MODE_RTC_64,
	MTL_RXQn_OPERATION_MODE_RTC_32,
	MTL_RXQn_OPERATION_MODE_RTC_96,
	MTL_RXQn_OPERATION_MODE_RTC_128
};
