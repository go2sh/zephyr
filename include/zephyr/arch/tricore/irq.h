...
extern void z_tricore_irq_priority_set(unsigned int irq, unsigned int prio);

#define ARCH_IRQ_CONNECT(irq_p, priority_p, isr_p, isr_param_p, flags_p)                           \
	{                                                                                          \
		Z_ISR_DECLARE(irq_p, 0, isr_p, isr_param_p);                                       \
		z_tricore_irq_priority_set(irq_p, priority_p);                                     \
	}

#define ARCH_IRQ_DIRECT_CONNECT(irq_p, priority_p, isr_p, flags_p)                                 \
	{                                                                                          \
		Z_ISR_DECLARE_DIRECT(irq_p, ISR_FLAG_DIRECT, isr_p);                               \
		z_tricore_irq_priority_set(irq_p, priority_p, flags_p);                            \
	}
